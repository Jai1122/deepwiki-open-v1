...
class ContextManager:
    ...
    def build_prompt(
        ...
        This implementation uses a proactive token budgeting approach.
        """
        ...
        if file_content and remaining_budget > 0:
            ...
            truncated_file_content = self._truncate_text(file_content, file_content_budget)

            # Update the file content string and remaining budget
            file_content_str = template.format(content=truncated_file_content)
            remaining_budget -= count_tokens(file_content_str, self.is_ollama)
            logger.info(f"Token count after adding file content: {count_tokens(file_content_str, self.is_ollama)}. Remaining budget: {remaining_budget} tokens.")
        ...
        logger.info(f"Final prompt token count: {final_token_count}")
        return final_prompt
