...
export async function GET() {
  try {
    const targetUrl = `${TARGET_SERVER_BASE_URL}/models/embeddings`;
    ...
    const embeddingConfig = await backendResponse.json();
    return NextResponse.json(embeddingConfig);
  } catch (error) {
    console.error('Error fetching embedding model configurations:', error);    
